...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    function rectTranslate (d) {
      ...
      var chartXY = [];
      chartXY[0] = chart().xScale(getOrAddDate(d));
      chartXY[1] = chart().yScale(utils.scaleMgdl(top));

      if (chartXY[0] == null || chartXY[1] == null) {
        console.warn('Bad Translate Data: ', chartXY);
      }
      return 'translate(' + chartXY[0] + ',' + chartXY[1] + ')';
    }
  ...
  function appendTreatments (treatment, arc) {
    ...
    var chartXY = [];
    chartXY[0] = chart().xScale(getOrAddDate(treatment));
    chartXY[1] = chart().yScale(client.sbx.scaleEntry(treatment));
    ...
    var treatmentDots = chart().focus.selectAll('treatment-insulincarbs')
      ...
      .attr('transform', 'translate(' + chartXY[0] + ', ' + chartXY[1] + ')')
